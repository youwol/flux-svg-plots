...
export function drawShapes( svgGroup , geoms , svgElements , ){
    ...
    let selection = select(svgGroup.node() ).selectAll("g.entity").data(geoms, d => d.id)
    ...
    updateGroups.each( function(d){        
        ...
        [...elem.attributes].filter( attr => attr.nodeName!='id').forEach( attr => node.setAttribute(attr.nodeName,attr.nodeValue) )
        while(c  ){
            node.appendChild( c )
            c = elem.firstChild 
        }
    ...
    let newGroups = selection.enter().append( d=>svgElements(d))             
    .attr("class", (d) => d.classes.join(" ") )
...
export class Plottable{
    ...
    formatAction( actionName, that, d, thisSvg,  mouse ) { 
        
        return { action: actionName, d3Selection : thisSvg, data: d, serie:that,
            mouse: mouse, event : d3Event  }
    }

    plugActions(groups, entities$ ) {
        let that = this 
        var drag = d3Drag();
        new Array("click","dblclick","mousedown","mouseover","mouseout","mouseup").forEach( (action) =>
            groups.on(action,  function(d) { 
                
                d3Event.stopPropagation()
                entities$.next( that.formatAction(action, that, d, select(this),  mouse(this) ) )
            })
        )
        new Array("start","drag","end").forEach( (action) =>
            drag.on(action,      function(d) { 
                entities$.next( that.formatAction(action,that, d, select(this),  mouse(this) ) )
            })
        )
        groups.call(drag) 
